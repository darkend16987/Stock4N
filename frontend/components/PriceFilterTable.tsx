      ...
      <div className="px-6 py-4 border-b border-slate-700/50 space-y-3">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <span className="text-xs text-slate-500">
            {data.length > 0 ? `${data.length} ma khop dieu kien` : 'Chua co ket qua'}
          </span>
        </div>
          ...
          <div className="bg-violet-500/10 border border-violet-500/30 rounded-lg px-4 py-3">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 text-sm">
                <span className="text-violet-300 font-medium">Dang quet thi truong...</span>
                <div className="text-xs text-slate-400 mt-1 flex gap-4 flex-wrap">
                  {scanStatus.processed !== undefined && (
                    <span>Da xu ly: <strong className="text-slate-300">{scanStatus.processed}</strong> ma</span>
                  )}
                  {scanStatus.batch && (
                    <span>Batch: <strong className="text-slate-300">{scanStatus.batch}</strong></span>
                  )}
                  {scanStatus.matches !== undefined && (
                    <span>Khop: <strong className="text-emerald-400">{scanStatus.matches}</strong></span>
                  )}
                  {scanStatus.last_update && (
                    <span>Cap nhat: {scanStatus.last_update.substring(11, 19)}</span>
                  )}
                </div>
              </div>
            </div>
      ...
      {data.length > 0 ? (
        <>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-slate-700/50">
                  ...
                  <ThSort col="Close" label="Gia" onSort={handleSort} sortKey={sortKey} sortAsc={sortAsc} align="right" />
                  <ThSort col="SMA200" label="SMA200" onSort={handleSort} sortKey={sortKey} sortAsc={sortAsc} align="right" />
                  <ThSort col="Pct_Below_SMA200" label="% Duoi SMA" onSort={handleSort} sortKey={sortKey} sortAsc={sortAsc} align="right" />
                  ...
                  <th className="px-4 py-3 text-left text-[10px] font-semibold text-slate-500 uppercase tracking-wider">Ngay Quet</th>
                </tr>
              </thead>
              <tbody>
          ...
          {sortedData.length > showCount && (
            <div className="text-center py-3 border-t border-slate-700/50">
              ...
              >
                Xem them ({sortedData.length - showCount} con lai)
              </button>
            </div>
          )}
        </>
        ...
        !isRunning && (
          <div className="px-6 py-12 text-center text-slate-500">
            ...
            <p className="text-sm">Chua co ket qua. Nhan <strong>&quot;Bat Dau Quet&quot;</strong> de quet toan thi truong.</p>
            <p className="text-xs mt-1 text-slate-600">Quet ~1700 ma co the mat vai gio. Tien trinh duoc luu tu dong.</p>
          </div>
        )
      )}
    </div>
