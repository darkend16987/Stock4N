"""
AI Reasoner - Phân tích sâu bằng LLM
Dùng Claude API để lập luận, tổng hợp và đề xuất từ dữ liệu đã thu thập.
...
# ──────────────────────────────────────
# Optional: import anthropic only if available
# ──────────────────────────────────────
try:
    import anthropic
...
class AIReasoner:
    """
    Sử dụng Claude API để phân tích sâu cổ phiếu.

    Cần thiết lập ANTHROPIC_API_KEY trong .env hoặc environment variable.
...
Cấu trúc output: Tóm tắt → Điểm mạnh → Rủi ro → Khuyến nghị."""
    ...
    def __init__(self, api_key: str | None = None):
        ...
        if not key:
            logger.warning("ANTHROPIC_API_KEY not set. AI reasoning disabled.")
            self._client = None
            return
    ...
    def _build_stock_prompt(self, symbol: str, d: dict) -> str:
        ...
        if d.get("market_sentiment"):
            lines.append(f"**Market breadth:** {d['market_sentiment']} "
                         f"({d.get('pct_above_both', '?'):.0%} mã trên SMA10&20)")
    ...
    def _fallback_analysis(self, symbol: str, d: dict) -> str:
        ...
        return (
            ...
            f"(AI analysis yêu cầu ANTHROPIC_API_KEY)"
        )
    ...
    def _fallback_portfolio(self, portfolio: list) -> str:
        return f"[Rule-based] Danh mục gồm {len(portfolio)} mã. (AI analysis yêu cầu ANTHROPIC_API_KEY)"
